...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/test/testSysUser/">test列表</a></li>
		<li class="active"><a href="${ctx}/test/testSysUser/form?id=${testSysUser.id}">test<shiro:hasPermission name="test:testSysUser:edit">${not empty testSysUser.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="test:testSysUser:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="testSysUser" action="${ctx}/test/testSysUser/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<sys:treeselect id="office" name="office.id" value="${testSysUser.office.id}" labelName="office.name" labelValue="${testSysUser.office.name}"
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="loginDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${testSysUser.loginDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
		...
		<div class="form-actions">
			<shiro:hasPermission name="test:testSysUser:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
